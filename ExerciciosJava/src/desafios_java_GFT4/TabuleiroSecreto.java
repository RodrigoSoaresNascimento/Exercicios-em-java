...
public class TabuleiroSecreto {
	...
	public static void main(String[] args) {
		...
		int tamanhoTabuleiro, NumeroJogadas;
		
		String [] q = entrada.nextLine().split(" ");
		
		tamanhoTabuleiro = Integer.parseInt(q[0]);
		
		NumeroJogadas = Integer.parseInt(q[1]);
		
		int[][] matriz = new int[tamanhoTabuleiro][tamanhoTabuleiro];
		...
		for(int i = 0; i < NumeroJogadas; i++  ) {
			 ...
			 List <String> variaveis = Arrays.asList(entrada.nextLine().split(" "));
			 
			 
			 int tipoOp =  Integer.parseInt(variaveis.get(0));
			 int linha =  Integer.parseInt(variaveis.get(1));
			 int valor =  0;
			 
			 if(variaveis.size() == 3) {
				 
				 valor = Integer.parseInt(variaveis.get(2));
				 
			 }
			 switch(tipoOp) {
			 	
			 	case 1 : atribuiLinhaX(linha, valor, matriz);
			 	break;
			 	case 2 : atribuiColunaX(linha, valor, matriz); 
			 	break;
			 	case 3 : imprimirFrequenteLinhaX(linha, matriz);
			 	break;
			 	case 4 :  imprimirFrequenteColunaX(linha, matriz); 
			 	break;
	...
	private static void imprimirFrequenteColunaX(int col,
		...
		Map <Integer, Integer> tabuleiro = new HashMap<>();
		
		tabuleiro.put(matriz[0][col-1], 1);
		
		 for (int i = 1; i < matriz.length; i++) {
	            if(tabuleiro.containsKey(matriz[i][col-1])){
	            	
	            	
	            	tabuleiro.put(matriz[i][col-1], tabuleiro.get(matriz[i][col-1])+1);
	            	
	            } else {
	            	
	            	tabuleiro.put(matriz[i][col-1], 1);
	            }
	        }
	    
	        var key = tabuleiro.entrySet().stream().max((entry1, entry2) -> entry1.getValue() > entry2.getValue() ? 1 : -1).get().getKey();
	        
	        if(tabuleiro.get(key) == 1){
	        	
	            key = tabuleiro.entrySet().stream().max((entry1, entry2) -> entry1.getKey() > entry2.getKey() ? 1 : -1).get().getKey();
	        }
	...
	private static void imprimirFrequenteLinhaX(int linha, int[][] matriz) {
