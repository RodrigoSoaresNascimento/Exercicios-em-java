...
texto entre dois caracteres de sublinhado, '_'. 
...
itálico um pedaço de texto no HTML, você deve iniciar este pedaço com a tag <i>
e terminá-lo com a tag </i>. Para texto em negrito, inicie com <b> e termine
com </b>. */
...
public class WebloggerBrasil {
	...
	public static void main(String[] args) throws IOException {
		...
		while((sentences = src.readLine()) != null) {
			
			boolean init = true;
			
			while(sentences.contains("_") || sentences.contains("*")) {
				if(sentences.contains("_") && init) {
				sentences = sentences.replaceFirst("_", "<i>");
				init = false;
			}else if(sentences.contains("_") && !init) {
				sentences = sentences.replaceFirst("_", "</i>");
				init = true;
				
			}else if(sentences.contains("*") && init ) {
				sentences = sentences.replaceFirst("\\*", "<b>");
				init = false;
